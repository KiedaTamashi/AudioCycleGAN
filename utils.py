...
def get_scheduler(optimizer, opt):
    ...
    if opt.lr_policy == 'linear':
        def lambda_rule(epoch):
            lr_l = 1.0 - max(0, epoch + opt.save_freq - opt.niter) / float(opt.niter_decay + 1)
            return lr_l
...
class BaseModel(ABC):
    ...
    def __init__(self, opt):
        ...
        self.save_dir = opt.summary_path  # save all the checkpoints to save_dir
    ...
    def setup(self, opt):
        ...
        if not self.isTrain or opt.continue_train:
            load_suffix = 'iter_%d' % opt.load_iter if opt.load_iter > 0 else opt.n_epochs
...
class AudioPool():
    """This class implements an buffer that stores previously generated .
    ...
    def __init__(self, pool_size):
        ...
        Parameters:
            pool_size (int) -- the size of buffer, if pool_size=0, no buffer will be created
        """
    ...
    def query(self, audios):
        ...
        return_audios = []
        for audio in audios:
            audio = torch.unsqueeze(audio.data, 0) # add dims
            if self.num < self.pool_size:   # if the buffer is not full; keep inserting current audios to the buffer
                self.num = self.num + 1
